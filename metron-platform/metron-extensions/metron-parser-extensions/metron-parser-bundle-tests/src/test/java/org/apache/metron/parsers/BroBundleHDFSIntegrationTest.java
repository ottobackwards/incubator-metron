...
public class BroBundleHDFSIntegrationTest extends BaseIntegrationTest {
  ...
  static final private String sensorType = "bro";
  static final private String ERROR_TOPIC = "parser_error";
  static private MRComponent mrComponent;
  static private FileSystem fileSystem;
  private List<byte[]> inputMessages;
  ...
  /**
   * Setup the test - Copy the resources to target area - Create the hdfs instance backed by that
   * area - Setup the BundleProperties with the correct information so that the BundleSystem can
   * talk to hdfs
   */
  @BeforeClass
  public static void setup() {
    ...
    mrComponent.start();
    Configuration configuration = mrComponent.getConfiguration();
    ...
    try {
      ...
      BundleProperties properties = BundleProperties
          .createBasicBundleProperties("./target/remote/zookeeper/bundle.properties",
              new HashMap<>());
      if (properties == null) {
        throw new RuntimeException("Failed to load BundleProperties");
      }

      String hdfsPrefix = configuration.get("fs.defaultFS");
      properties
          .setProperty(BundleProperties.BUNDLE_LIBRARY_DIRECTORY, hdfsPrefix + "/extension_lib/");
      ...
      if (!fileSystem.mkdirs(new Path("/work/"),
          new FsPermission(FsAction.READ_WRITE, FsAction.READ_WRITE, FsAction.READ_WRITE))) {
        System.out.println("FAILED MAKE DIR");
      }
      fileSystem
          .copyFromLocalFile(new Path("./target/remote/metron/extension_lib/"), new Path("/"));
      fileSystem.copyFromLocalFile(new Path("./target/remote/zookeeper/bundle.properties"),
          new Path("/work/"));
  ...
  /**
   * Integration test Setup zookeeper - including BundleProperties Setup the parser - storm and
   * kafka Run some messages through and verify the result
   *
  ...
  public void testIntegration() throws Exception {
    ...
    ConfigUploadComponent configUploadComponent = new ConfigUploadComponent()
        ...
        .withGlobalConfigsPath("./target/remote/zookeeper/").withParserConfigsPath(
            "../metron-parser-bro-extension/metron-parser-bro/"
                + TestConstants.THIS_PARSER_CONFIGS_PATH);

    ParserTopologyComponent parserTopologyComponent = new ParserTopologyComponent.Builder()
        .withSensorType(sensorType).withTopologyProperties(topologyProperties)
    ...
    ComponentRunner runner = new ComponentRunner.Builder().withComponent("zk", zkServerComponent)
        ...
        .withMillisecondsBetweenAttempts(5000).withNumRetries(10)
        .withCustomShutdownOrder(new String[]{"org/apache/storm", "config", "kafka", "zk"}).build();
    runner.start();
    ...
    try {
      ...
      if (result.failed()) {
        ...
        buffer.append(String.format("%d Valid Messages Processed", outputMessages.size()))
            .append("\n");
  ...
  private void dumpParsedMessages(List<byte[]> outputMessages, StringBuffer buffer) {
  ...
  private KafkaProcessor<List<byte[]>> getProcessor() {
    ...
    return new KafkaProcessor<>().withKafkaComponentName("kafka")
        .withReadTopic(Constants.ENRICHMENT_TOPIC).withErrorTopic(ERROR_TOPIC)
        .withValidateReadMessages(new Function<KafkaMessageSet, Boolean>() {
          @Override
          public Boolean apply(@Nullable KafkaMessageSet messageSet) {

            return messageSet == null ? Boolean.FALSE
                : (messageSet.getMessages().size() + messageSet.getErrors().size() == inputMessages
                    .size());
          }
        }).withProvideResult(new Function<KafkaMessageSet, List<byte[]>>() {
          @Override
          public List<byte[]> apply(@Nullable KafkaMessageSet messageSet) {
            return messageSet == null ? new LinkedList<>() : messageSet.getMessages();
          }
        });
  }

  private List<ParserValidation> getValidations() {
    return new ArrayList<ParserValidation>() {{
      add(new PathedSampleDataValidation(
          "../metron-parser-bro-extension/metron-parser-bro/src/test/resources/data/parsed/test.parsed"));
    }};
  }

  private String getSampleDataPath() throws Exception {
