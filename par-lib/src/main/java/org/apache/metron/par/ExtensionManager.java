...
public class ExtensionManager {
     ...
     */
    public static void discoverExtensions(final Bundle systemBundle, final Set<Bundle> narBundles) throws NotInitializedException {
        checkInitialized();
     ...
     *
     * @param parProperties a NiFiProperties instance which will be used to obtain the default NAR library path,
     *                       which will become the working directory of the returned bundle
     * @return a bundle for the system class loader
     */
    public static Bundle createSystemBundle(final FileSystemManager fileSystemManager, final ParProperties parProperties) throws FileSystemException, URISyntaxException {
        final ClassLoader systemClassLoader = ClassLoader.getSystemClassLoader();

        final String parLibraryDirectory = parProperties.getProperty(ParProperties.PAR_LIBRARY_DIRECTORY);
        if (StringUtils.isBlank(parLibraryDirectory)) {
            throw new IllegalStateException("Unable to create system bundle because " + ParProperties.PAR_LIBRARY_DIRECTORY + " was null or empty");
        }
        final URI parLibraryDirURI = parProperties.getParLibraryDirectory();
        FileObject parDir = fileSystemManager.resolveFile(parLibraryDirURI);
